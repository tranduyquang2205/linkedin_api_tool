...
class Linkedin(object):
    ...
    def _delete(self, uri, evade=default_evade, base_request=False, **kwargs):
        """POST request to Linkedin API"""
        evade()

        url = f"{self.client.API_BASE_URL if not base_request else self.client.LINKEDIN_BASE_URL}{uri}"
        return self.client.session.delete(url, **kwargs)    
    ...
    def search_people(
        ...
        **kwargs,
        ...
        data = self.search(params,limit, **kwargs)
            ...
            results.append(
                {
                    ...
                    "work_info":item.get("headline", {}).get("text"),
                    "linkedin_url":item.get("navigationUrl"),
    ...
    def get_profile_contact_info(self, public_id=None, urn_id=None):
        ...
        return contact_info
    def get_profile_phone(self, public_id=None, urn_id=None):
        res = self._fetch(
            f"/identity/profiles/{public_id or urn_id}/profileContactInfo"
        )
        data = res.json()
        
        if len(data.get("phoneNumbers", [])) < 1 or "phoneNumbers" not in data:
            return
        phone_list = []
    ...
    def get_profile_connections(self, urn_id):
        ...
        :rtype: list
        """
        if urn_id == self.get_user_profile()['miniProfile']['publicIdentifier'] or urn_id == None:
            return self.search_people(network_depth="F")
    ...
    def update_info(self, first_name,last_name,info,state):
            params = {"action": "saveV2",
            "decorationId": "com.linkedin.voyager.dash.deco.identity.profile.ProfileEditFormPageSaveResponseWithTopCardData-3"
            }
            uid = self.get_user_profile()['miniProfile']['publicIdentifier']
            list_info = {
    "formElementInputs": [
        {
        "formElementInputValues": [
            {
            "textInputValue": first_name
            }
        ],
        "formElementUrn": "urn:li:fsd_profileEditFormElement:(TOP_CARD,urn:li:fsd_profile:"+uid+",/firstName)"
        },
        {
        "formElementInputValues": [
            {
            "textInputValue": last_name
            }
        ],
        "formElementUrn": "urn:li:fsd_profileEditFormElement:(TOP_CARD,urn:li:fsd_profile:"+uid+",/lastName)"
        },
        {
        "formElementInputValues": [
            {
        ...
        "formElementUrn": "urn:li:fsd_profileEditFormElement:(TOP_CARD,urn:li:fsd_profile:"+uid+",/headline)"
        },
        {
        "formElementInputValues": [
            {
        ...
        "formElementUrn": "urn:li:fsd_profileEditFormElement:(TOP_CARD,urn:li:fsd_profile:"+uid+",/geoLocation)"
        }
    ],
    "trackingId": self.generateTrackingIdAsCharString()
            ...
            res = self._post(
                f"/voyagerIdentityDashProfileEditFormPages",
                params=params,
                data=json.dumps(list_info),
            )
    ...
    def send_message(self, message_body, conversation_urn_id=None, recipients=None):
        ...
        return res.json()
    def delete_message(self,conversation_urn_id):
       
        res = self._delete(
            f"/messaging/conversations/{conversation_urn_id}",
        )
        return True
